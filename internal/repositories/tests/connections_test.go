...
func TestConnectionsGetAllByIssuerID(t *testing.T) {
	...
	t.Run("should get 0 connections for a non existing issuerDID", func(t *testing.T) {
		conns, _, err := connectionsRepo.GetAllWithCredentialsByIssuerID(ctx, storage.Pgx, *userDID, &ports.NewGetAllConnectionsRequest{Query: ""})
		require.NoError(t, err)
	...
	t.Run("should get 1 connection for a the given issuerDID and valid query, just beginning", func(t *testing.T) {
		conns, _, err := connectionsRepo.GetAllWithCredentialsByIssuerID(ctx, storage.Pgx, *issuerDID, &ports.NewGetAllConnectionsRequest{Query: "did:"})
		require.NoError(t, err)
		assert.Equal(t, 1, len(conns))
	})
	...
	t.Run("should get 1 connection for a the given issuerDID and a query with some chars in the middle of a string", func(t *testing.T) {
		conns, _, err := connectionsRepo.GetAllWithCredentialsByIssuerID(ctx, storage.Pgx, *issuerDID, &ports.NewGetAllConnectionsRequest{Query: "H7XAw"})
		require.NoError(t, err)
		assert.Equal(t, 1, len(conns))
	})
	...
	t.Run("should get 1 connection for a the given issuerDID and a query with some chars in the middle of a string and other words", func(t *testing.T) {
		conns, _, err := connectionsRepo.GetAllWithCredentialsByIssuerID(ctx, storage.Pgx, *issuerDID, &ports.NewGetAllConnectionsRequest{Query: "H7XAw other words"})
		require.NoError(t, err)
		assert.Equal(t, 1, len(conns))
	})
